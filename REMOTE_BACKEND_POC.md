...
This fork includes a **proof-of-concept** daemon that runs Fridex's backend logic in a separate process (intended for WSL2/Linux), exposing a simple **line-delimited JSON-RPC** protocol over TCP.
...
cargo run --bin codex_monitor_daemon -- \
  ...
  --data-dir "$HOME/.local/share/fridex-daemon" \
  --token "$TOKEN"
```
